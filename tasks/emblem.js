...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('emblem', 'Compile emblem templates and partials.', function() {
      ...
      f.src.filter(function(filepath) {
        ...
        .forEach(function(filepath) {
          ...
          try {
            ...
            // try compile emblem or handlebars
            if (path.extname(filepath) =='.emblem') {
              compiled = Emblem.precompile(Handlebars, src, compilerOptions);
            } else if (path.extname(filepath) =='.hbs') {

              // parse the handlebars template into it's AST
              var ast = processAST(Handlebars.parse(src));
              compiled = Handlebars.precompile(ast, compilerOptions);

            } else {
              grunt.fail.warn('Can not compile file of type ' + path.extname(filepath));
            }
